...
jQuery(document).ready(function ($) {
  ...
  $("#virtualForm").submit(function (e) {
    ...
    let apiUrl = `${root_url}v1/virtualApi/createRender`;
    ...
    xhr.onload = function () {
      if (xhr.status === 200) {
        ...
        if (response.status) {
          ...
          $("#imagePort").attr("src", responseData.result_image_url);
          $("#downloadImage").attr("href", responseData.result_image_url);
  ...
  $(".delete-link").each(function () {
    $(this).click(function () {
      ...
      let elementId = $(this).attr("data-id");
      let storedData = localStorage.getItem("aiImages");
      let body = {'post_id':elementId};
      let apiUrl = `${root_url}v1/virtualApi/deleteRender`;
      $.ajax({
        type: "post",
        url: apiUrl,
        data: body,
        ...
        error: function (error) {
          console.error("Request failed. Status: " + error.status);
           notification("success", "Success", "An error occured, please try again later");
